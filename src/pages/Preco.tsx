  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      <KpiCard
        ...
        value={`${formatBRL(result.precoMedioBase)}/kg`}
        accent="violet"
      />
      <KpiCard
        label={`Preço médio • ${result.compLabel}`}
        value={`${formatBRL(result.precoMedioComp)}/kg`}
        accent="blue"
      ...
      <KpiCard
        ...
        value={`${result.variacaoTotal >= 0 ? "+" : "−"}${formatBRL(Math.abs(result.variacaoTotal))}/kg`}
  ...
  sentences.push(
    <>
      ...
      <span className="font-semibold">
        {formatBRL(Math.abs(result.variacaoTotal))}/kg ({formatPct(Math.abs(result.variacaoPct))})
      </span>{" "}
  ...
  if (top && top.deltaPrecoRs !== 0) {
    sentences.push(
      <>
        ...
        <span className="font-semibold">
          {top.deltaPreco >= 0 ? "+" : "−"}
          {formatBRL(Math.abs(top.deltaPreco))}/kg
        </span>{" "}
...
function RankingSection({
  ...
  const columns: DataTableColumn<Row>[] = [
    ...
    { key: "precoBase", label: "Preço Base", align: "right", format: (v) => formatBRL(Number(v)) },
    { key: "precoComp", label: "Preço Comp", align: "right", format: (v) => formatBRL(Number(v)) },
    {
      key: "deltaPreco",
      label: "Δ Preço (R$/kg)",
      align: "right",
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 16, left: 8, bottom: 8 }}>
            ...
            <RTooltip
              ...
              formatter={(value: number, name: string) => [`${formatBRL(value)}/kg`, name]}
            />
